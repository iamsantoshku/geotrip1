...
import airplaneLoader from "/airplaneLoader.gif"
  ...
  const handleFlightBooking = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        BACKENDURL + "/api/v1/bookings/flight/checkout-session/" + id,
        {
          method: "POST",
          headers: {
      ...
      // optional take data from backend and localstorage
      ...
      //     },
      //     body: JSON.stringify({
      //       token: localStorage.getItem("token"), // Get the token directly from local storage
      //       bookingUsersData: formData, // Data from the form
      //       selectedSeats: selectedSeatsArray, // Array of selected seats
      //       user: JSON.parse(localStorage.getItem("user")), // Get the user details from local storage
      //     }),
      //   }
      // );
