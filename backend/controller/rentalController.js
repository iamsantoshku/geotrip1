...
//     // Create a new rental property with the file path stored
...
//       images: req.file.path, // Store the file path in the database
...
//   try {
//     const rental = await RentalList.findById(req.params.id);
//     if (!rental) {
//       return res.status(404).json({ message: "rental not found" });
//     }
//     res.status(200).json({ data: rental });
//   } catch (error) {
//     res.status(500).json({ message: "Failed to retrieve rental", error: error.message });
//   }
// };
...
//     // Fetch the rental from the database using the ID
//     const rental = await RentalList.findById(id);
...
//     res.status(200).json({ message: "Rental fetched successfully!", data: rental });
//   } catch (error) {
//     // Handle errors
//     console.error("Error fetching rental:", error);
//     res.status(500).json({ message: "An error occurred while fetching the rental.", error });
//   }
// };
